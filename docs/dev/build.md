...
# -g=REGISTRY, --registry=REGISTRY              Docker image registry (docker registry and username). Default is "keksmd".
# -r=REPO, --repo=REPO                          Docker image repository. Default is "task_decomposition".
...
# Or:
./scripts/build.sh -p=arm64 -b=ubuntu:24.04 -g=keksmd -r=task_decomposition -v=1.0.0 -s=-arm64 -d=./Dockerfile -t=. -u -c
```
...
docker build \
    --progress plain \
    -t keksmd/task_decomposition:latest \
    .

# Push image to Docker Registry:
docker push [IMG_FULLNAME]
# For example:
docker push keksmd/task_decomposition:latest
```
...
docker buildx build \
    --progress plain \
    --platform linux/amd64,linux/arm64 \
    --cache-from=type=registry,ref=keksmd/task_decomposition:cache-latest \
    --cache-to=type=registry,ref=keksmd/task_decomposition:cache-latest,mode=max \
    -t keksmd/task_decomposition:latest \
    --push \
    .

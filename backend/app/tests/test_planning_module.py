...
from app.planning.data.loaders import FewShotDatasetLoader, JsonArtifactRepository, ToolRegistryLoader
from app.planning.data.taskcraft import build_processed_dataset, build_tool_registry_from_raw, convert_taskcraft_row
...
from app.planning.schemas import InferenceRequest, PlannerExample


class DummyTextLLMClient:
...
def test_normalizers_taskcraft_and_loaders(tmp_json_artifacts):
    assert safe_to_obj('{"a": 1}') == {"a": 1}
    assert clean_plan_text("Here is the plan of action that I will follow to solve the task:\n```\n1. Search\n2. Open\n```") == "1. Search\n2. Open"
    assert split_plan_steps("1. Search\n2. Open") == ["Search", "Open"]
    assert normalize_action_arguments("crawl_pages", {"url": "https://example.com", "file_path": "/tmp/x"}) == {"url": "<retrieved_url>"}
    ...
    output = normalize_planner_output({"plan": "1. Search\n2. Open", "actions": [a.model_dump() for a in actions]})
        ...
        "ans_from_agent": str(
            {
                "trace": {
                    "plan": "1. Search\n2. Open the page",
                    "actions": [
                        {"tool_name": "web_search", "arguments": {"query": "received date"}},
                        {"tool_name": "crawl_pages", "arguments": {"url": "https://example.com"}},
                        {"tool_name": "final_answer", "arguments": {"answer": "x"}},
                    ],
                }
            }
        ),
    ...
    assert list(processed_df.columns) == ["task", "plan", "actions", "n_plan_steps", "n_actions"]
...
def test_prompt_parser_service_and_route(tool_specs, planner_examples):
    prompt_builder = PlanningPromptBuilder()
    request = InferenceRequest(task="Find the received date.", available_tools=tool_specs, few_shot_examples=planner_examples[:2])
    ...
    with __import__("pytest").raises(PromptBuildError):
        prompt_builder.build(InferenceRequest(task="   ", available_tools=tool_specs))
    ...
    with __import__("pytest").raises(PredictionParseError):
    ...
    service = PlanningService(
        ...
        config=PlanningConfig(include_prompt_debug=True, include_raw_response=True),
    ...
    result2 = service.predict_from_parts(task="Find the received date.", available_tools=tool_specs, few_shot_examples=planner_examples[:1])
    assert isinstance(result2.prediction, PlannerOutput)

    selector = FewShotSelector()
    selected = selector.select_by_indices(planner_examples, [3, 100], fallback_count=3)
    ...
    response = client.post(
        ...
        json={
            ...
            "few_shot_examples": [example.model_dump() for example in planner_examples[:1]],
        },
    )
